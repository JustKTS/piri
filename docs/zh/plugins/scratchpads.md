...
3. **切换检查**: 执行 toggle 时，如果窗口不是浮动状态，直接 focus 到该窗口并立即返回，不执行动画
4. **显示**: 将窗口移动到当前聚焦的输出和工作区，按配置的方向和大小定位，并聚焦窗口
5. **隐藏**: 将窗口移动到屏幕外，智能恢复之前的焦点

**跨 workspace 和 monitor**: 无论 scratchpad 窗口原本在哪个工作区或显示器上，都会自动移动到当前聚焦的位置。

> **注意**: Scratchpad 功能仅对浮动窗口生效。`auto_hide_on_focus_loss` 等特性也仅作用于浮动状态的窗口。如果窗口被改为平铺模式，toggle 时只会直接聚焦该窗口。
...
- ✅ **非浮动窗口处理**: toggle 时若窗口非浮动，直接 focus 而不执行动画
...
- ✅ **失去焦点自动隐藏**: 窗口失去焦点时自动隐藏（`auto_hide_on_focus_loss` 选项，仅对浮动窗口生效）